...
impl DemoApp {
    fn from_window(window: Arc<Mutex<Option<Window>>>) -> App {
        App::new(Self {
...
impl ImplApp for DemoApp {
    ...
    fn get_browser_process_handler(&self) -> Option<BrowserProcessHandler> {
        Some(DemoBrowserProcessHandler::from_window(self.window.clone()))
    }
}
...
impl DemoBrowserProcessHandler {
    fn from_window(window: Arc<Mutex<Option<Window>>>) -> BrowserProcessHandler {
...
fn main() {
    ...
    let mut app = DemoApp::from_window(window.clone());
